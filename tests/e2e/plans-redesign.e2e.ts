...
test('тарифы показывают текущий доступ, точную сумму и последствия перехода', async ({ page }, testInfo) => {
  ...
  await expect(summary).toContainText('Устройства')
  ...
  if (!mobile) {
    const confirmation = page.getByRole('dialog', { name: 'Перейти на тариф «E2E 90 дней»?' })
    const purchaseSummary = confirmation.getByRole('region', { name: 'Итог покупки' })
    await expect(purchaseSummary).toContainText('Срок')
    await expect(purchaseSummary).toContainText('Устройства')
    await expect(purchaseSummary).toContainText('К оплате сейчас')
    await expect(purchaseSummary).toContainText('Автопродление')
    await confirmation.getByRole('button', { name: /Оплатить/ }).click()
  }
