...
test('мобильная навигация переносит второстепенные разделы вниз', async ({ page }, testInfo) => {
  ...
  await expect(navigation.getByRole('link', { name: 'VPN' })).toBeVisible()
  await expect(navigation.getByRole('link', { name: 'Тариф' })).toBeVisible()
  await expect(navigation.getByRole('link', { name: 'Помощь' })).toBeVisible()
  ...
  await expect(moreMenu.getByRole('link', { name: 'Настройки' })).toBeVisible()
...
test('мобильный выбор тарифа открывает оплату отдельным окном', async ({ page }, testInfo) => {
  ...
  await expect(checkout.getByText('Включено 5, далее +100 ₽ за устройство на весь срок')).toBeVisible()
  await deviceLimit.fill('8')
  await deviceLimit.press('Enter')
  await expect(deviceLimit).toHaveValue('8')
  await expect(checkout.getByText('130 ₽ + 300 ₽ за 3 доп.')).toBeVisible()
  await expect(deviceSelector.getByText('430 ₽', { exact: true })).toBeVisible()
...
test('активное подключение показывает добавление приложения и управление устройствами', async ({ page }) => {
  ...
  await expect(autoRenewal.getByText('130 ₽ · 7 дн.', { exact: true })).toBeVisible()
...
test('рулетка отправляет только один запрос и показывает результат', async ({ page }) => {
  ...
  const hub = page.getByRole('button', { name: /Получить подарок/ })
