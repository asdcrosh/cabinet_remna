...
На чистом сервере сначала устанавливается только `cabinetctl`:
...
- проверить здоровье сервера и бэкапов;
- после установки обновлять систему, менять `.env`, смотреть логи и управлять бэкапами.

### 3. Установите кабинет

В меню `cabinetctl` выберите:
...
В `cabinetctl` выберите `Здоровье системы`, затем откройте:
...
`cabinetctl nginx` или запустите скрипт настройки nginx из инструкции деплоя.
...
cabinetctl logs      # открыть меню логов
cabinetctl backups   # бэкапы, восстановление и S3
```

`cabinetctl update` скачивает свежий compose и image, применяет миграции,
проверяет запуск и не удаляет `.env` или volume базы.

В админке есть раздел `Система`. Там проверяются база кабинета, Remnawave API,
YooKassa, email, Telegram бот, свежесть бэкапа и S3-статус. Кнопка полной
проверки отправляет тестовое письмо на `SYSTEM_HEALTH_EMAIL_TO`.
...
В одном меню находятся создание бэкапа, восстановление, S3 и статус. Отдельных
пунктов “бэкап БД” нет: архив всегда полный и переносимый.
...
Проверить свежесть бэкапов и S3:
...
Откройте `Бэкапы`, выберите локальный архив или S3 по номеру и подтвердите
операцию словом `RESTORE`. Вручную вводить путь к архиву не требуется. Консоль
при необходимости установит Docker и AWS CLI.
...
```bash
FULL_BACKUP_DIR=/mnt/backups FULL_BACKUP_KEEP_DAYS=30 cabinetctl backup
```
...
Затем выберите `Бэкапы` → `S3`. Поддерживаются AWS S3 и S3-совместимые
хранилища с собственным endpoint. Доступы сохраняются только на сервере в
`/etc/remna-backup-s3.conf` с правами `600`. Можно включить автоматическую
загрузку каждого нового полного бэкапа.
