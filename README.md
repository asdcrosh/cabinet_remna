...
В `remnactl` выберите `Здоровье системы`, затем откройте:
...
`remnactl nginx` или запустите скрипт настройки nginx из инструкции деплоя.
...
remnactl health    # проверить систему
remnactl logs      # открыть меню логов
