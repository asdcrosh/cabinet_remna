...
Для настройки используется тот же единственный файл
`/opt/remnawave-cabinet/.env`. Второй `.env.provisioner` не нужен, а токен
Timeweb из общего файла получает только provisioning-worker:

```bash
cabinetctl provisioning
```

Обычно мастер спросит только Timeweb API token и основной домен нод. Ключ
шифрования генерируется автоматически, email берётся из настроек кабинета, а
образ RemnaNode и таймауты уже заполнены. IP панели определяется автоматически,
если домен Remnawave указывает на этот сервер; иначе мастер запросит его, чтобы
не открыть порт 2222 не тому адресу.
Мастер проверяет Timeweb и Remnawave безопасными GET-запросами и только после
этого включает и запускает `node-provisioning-worker`. TCP/XHTTP-шаблоны
выбираются непосредственно в админке.
...
| `cabinetctl backup-schedule` | Настроить автоматическое создание и отправку бэкапов |
| `cabinetctl backup-status` | Показать расписание и результат последнего запуска |
| `cabinetctl backup-notify-test` | Проверить доставку уведомления о бэкапе |
...
Мастер позволяет выбрать ежедневный запуск, рабочие дни или день недели, задать
время по часовому поясу сервера, срок локального хранения и отправку нового архива
в S3. Успешный результат или подробная ошибка отправляются в Telegram через
`TELEGRAM_BOT_TOKEN` и `TELEGRAM_NOTIFY_CHAT_ID` из `.env` кабинета. Расписание
работает через persistent systemd timer: пропущенный из-за
перезагрузки запуск выполнится после старта сервера. Статус доступен командой
`cabinetctl backup-status`, логи и тестовый запуск - внутри меню расписания.
Доставку сообщения без создания архива можно проверить командой
`cabinetctl backup-notify-test`.
