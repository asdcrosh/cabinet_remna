...
export function DuplicateMergeButton({
  ...
  return (
    <>
      <button type="button" className="btn-primary min-h-11 w-full justify-center px-3" onClick={() => setOpen(true)}>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100">
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex sm:justify-end">
            <button type="button" className="btn-secondary justify-center" onClick={() => setOpen(false)} disabled={loading}>
              Отмена
            </button>
            <button type="button" className="btn-primary justify-center" onClick={() => void mergeUsers()} disabled={loading}>
...
function AccountPreview({ title, email }: { title: string; email: string }) {
  return (
    <div className="min-w-0 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 dark:border-white/10 dark:bg-white/[0.04]">
