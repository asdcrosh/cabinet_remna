...
import { Gift, Sparkles, Ticket, WandSparkles } from 'lucide-react'
...
export function WelcomeOfferButton({ label, options }: { label: string; options: WelcomeBonusChoice[] }) {
  ...
  return (
    <>
      ...
      <Modal
        open={open}
        title="Выберите приветственный бонус"
        description="Можно получить только один вариант."
        onClose={() => {
        ...
        <div className="grid gap-2">
          {options.map((option) => (
            <button
              key={option.type}
              type="button"
              className="flex min-h-20 items-center gap-3 rounded-lg border border-slate-200 bg-white p-3 text-left transition hover:border-cyan-200 hover:bg-cyan-50 dark:border-white/10 dark:bg-white/[0.03] dark:hover:bg-cyan-500/10"
