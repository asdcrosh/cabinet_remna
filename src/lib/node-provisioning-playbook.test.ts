...
describe('node provisioning playbook safety', () => {
  ...
  it('issues certificates through the HTTP fallback without unsafe deployment', () => {
    ...
    expect(taskBlock('Install the staged SelfSteal certificate pair transactionally')).toContain('restore_previous_pair')
    expect(taskBlock('Install the staged SelfSteal certificate pair transactionally')).toContain("trap 'rc=$?")
