...
describe('production deployment security', () => {
  ...
  it('deploys verified cabinet releases by immutable sha image tag', () => {
    ...
    expect(updater).toContain('"${COMPOSE[@]}" pull --quiet')
    expect(updater).toContain("printf '\\rDownloading Docker images: %s sec.'")
    expect(updater).not.toContain('docker pull "${TARGET_CABINET_IMAGE}"')
